...
CREATE TABLE county (
   ...
   CONSTRAINT county_pkey
      PRIMARY KEY (county_id)) TYPE=INNODB;

CREATE TABLE new_counties (
   county_id DECIMAL,
   county_name VARCHAR(15),
   state CHAR(2)) TYPE=INNODB;
...
CREATE TABLE city (
   ...
   CONSTRAINT city_pkey
      PRIMARY KEY (city_id)) TYPE=INNODB;
...
CREATE TABLE attraction (
   ...
   CONSTRAINT attraction_pkey
      PRIMARY KEY (attraction_id)) TYPE=INNODB;
...
CREATE TABLE attraction_urls (
   id DECIMAL,
   url VARCHAR(40)) TYPE=INNODB;

CREATE TABLE attraction_names (
   id DECIMAL,
   name VARCHAR(35)) TYPE=INNODB;
...
CREATE TABLE song (
    ...
    artist VARCHAR(15),
    CONSTRAINT song_pk
        PRIMARY KEY (cd_id, track)) TYPE=INNODB;
...
CREATE TABLE worker_location (
   ...
   end_date TIMESTAMP) TYPE=INNODB;
...
CREATE TABLE part (
    ...
    current_inventory DECIMAL) TYPE=INNODB;
